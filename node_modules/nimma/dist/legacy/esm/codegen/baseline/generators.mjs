...
import { logicalExpression, binaryExpression, literal, memberExpression, numericLiteral, sequenceExpression, assignmentExpression, conditionalExpression, callExpression, identifier, unaryExpression, booleanLiteral, regExpLiteral, stringLiteral, nullLiteral } from '../ast/builders.mjs';
...
function generateMemberExpression(iterator, {
  ...
  const right = binaryExpression('!==', memberExpression(scope.path, iterator.state.pos === 0 ? numericLiteral(0) : iterator.feedback.fixed ? numericLiteral(iterator.state.pos) : binaryExpression('+', internalScope.pos, numericLiteral(iterator.state.pos)), true), literal(value));
...
function generateFilterScriptExpression(iterator, {
  ...
  const node = unaryExpression('!', rewriteESTree(esTree, iterator.state.fixed && iterator.state.pos > 0 && iterator.nextNode !== null ? iterator.state.pos + 1 : iterator.state.inverted && iterator.state.pos !== 0 ? iterator.state.pos - 1 : 0));
...
function rewriteESTree(node, pos) {
  switch (node.type) {
    ...
    case 'BinaryExpression':
      if (node.operator === 'in') {
        node.operator = '===';
        node.left = callExpression(memberExpression(node.right, identifier('includes')), [rewriteESTree(node.left, pos)]);
        ...
        node.left = callExpression(memberExpression(regExpLiteral(node.right.value, ''), identifier('test')), [rewriteESTree(node.left, pos)]);
        node.right = booleanLiteral(true);
      } else {
