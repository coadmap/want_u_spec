...
function generateMemberExpression(iterator, {
  ...
  if (iterator.feedback.bailed) {
    return builders.binaryExpression('!==', scope['default'].property, builders.literal(value));
  }

  if (iterator.state.inverted) {
    return builders.binaryExpression('!==', iterator.state.pos === 0 ? scope['default'].property : builders.memberExpression(scope['default'].path, builders.binaryExpression('-', scope['default'].depth, builders.numericLiteral(Math.abs(iterator.state.pos))), true), builders.literal(value));
  }
